...
[Signing](../../yid/deep-link-signing.md#signing) — the HTTPS example signed the
wrong bytes.
...
- **`app_sig` covers the canonical `yanezbio://sign?<query>` bytes, not the
  ...
  [Signing](../../yid/deep-link-signing.md#signing).
- **There is no callback-domain allowlist.** `method=post` delivers to whatever
  ...
  callback; see [Delivery Modes](../../yid/callback.md#delivery-modes).
- **iOS test builds register only their suffixed scheme** (`yanezbio-dev://`,
  ...
  [Custom Scheme (Deprecated)](../../yid/deep-link-signing.md#custom-scheme-deprecated).
...
- Two rejection messages: "Untrusted signing request" (signature or parameter
  ...
  environment). See [Common Errors](../../yid/deep-link-signing.md#common-errors).
