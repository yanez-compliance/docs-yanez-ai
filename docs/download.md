...
[signed deep link](yid/deep-link-signing.md).
...
failing silently. See [Deep Link Signing](yid/deep-link-signing.md#url-format) for
the full format.
...
delivered — see [Signing](yid/deep-link-signing.md#signing).
...
[Deep Link Signing](yid/deep-link-signing.md) — and open it the same way. The app
...
[Delivery Modes](yid/callback.md#delivery-modes).
...
| Signature verifies locally but not in the app | `app_sig` must be the **last** parameter, signed over the link exactly as delivered (same base, no re-ordering or re-encoding) — see [Signing](yid/deep-link-signing.md#signing). |
